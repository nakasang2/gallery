...
//
// It also maps exhibition subdomains onto the same routes (ユーザー決定 2026-08-09):
// `tokyo-expo.xibit360.art/painting` → `/_expo/tokyo-expo/painting`.
//
// Four cases, in order:
//   0. an exhibition subdomain → rewrite to `/_expo/{sub}/…` and stop. Checked FIRST
//      because those hosts serve one exhibition and nothing else: the dashboard, the
...
import { expoSubdomainFromHost } from '@/lib/expoHost'
...
export function middleware(req: NextRequest) {
  ...
  const sub = expoSubdomainFromHost(req.headers.get('host'))
  if (sub) {
    return NextResponse.rewrite(new URL(`/_expo/${sub}${pathname === '/' ? '' : pathname}${search}`, req.url))
  }
