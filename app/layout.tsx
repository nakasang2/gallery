...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <html lang={meta.bcp47} dir={meta.dir} data-script={meta.script}>
      <head>
        {/* Self-hosted (リリース前監査 #15) — this used to be a runtime <link> to
            fonts.googleapis.com, which sent every visitor's IP to Google before any
            consent decision (a German court found the same pattern unlawful under
            GDPR — DECISIONS 2026-08-21). public/fonts/site-fonts.css (generated by
            scripts/rebuild-self-hosted-fonts.py) covers the same families/weights,
            split into the same unicode-range buckets Google's own CSS2 API used, so
            a page that never shows Japanese text still never downloads the CJK
            chunks — self-hosting the WHOLE font rather than trimming it is what
            would have regressed page weight, not the fact of self-hosting. */}
