...
5. **検証ループ**: push 前にこのリポジトリの型チェック・ビルド・`npm run check:i18n`・`npm run check:css`・`npm run check:gates` を実行し、/code-review による別視点レビューを行う。ローカルではさらに ship スキルの手順に従う。
...
5.5 **番人を触ったら自己テストを通す（DECISIONS 2026-07-28）**: `npm run check:gates` が、合成入力を流して check:css / check:i18n が本当に検出しているかを確かめる（過去に×4、検出器が自分の検査範囲を取りこぼして「0件」が嘘になった）。**番人を追加・変更したら、その変更で守れるようになった入力をケースとして `scripts/check-gates.mjs` に足す。** 番人は git に登録されたものしか見ないので、未追跡ファイルがあると番人自身が落ちる（`git add` してから回す）。
