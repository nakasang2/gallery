# Xibit360 — 3Dバーチャルギャラリー 要件定義(ライト版)
...
- 名称: **Xibit360** — 自分だけの小さな世界を作り込む、という意味を込めて

## 2. ターゲットユーザー

| ユーザー | 課題 | Xibit360が提供する価値 |
|---|---|---|
...
- **3D鑑賞体験(来場者)**:
  ...
  - 作品クリックで拡大 + キャプション表示
- **公開・共有**: 固有URL発行(`xibit360.art/@username/exhibition`)、OGP対応
- **レスポンシブ**: PC / スマホ両対応
...
- ログアウト / **アカウント削除**(ギャラリー・作品・Storageファイルも削除)
- 画面: `/signup` `/signin` `/reset` の専用ページ。LPと `/demo` から導線
- **ゲスト移行**: ゲスト時にlocalStorageへ出展した作品を、登録直後に「アカウントへ取り込みますか?」と提案

### 10.2 ギャラリー(ギャラリー)の複数化

- **設計は複数ギャラリー対応、リリース時の上限は1人1個**。上限は将来プランごとに変えられるよう
...
- **編集**: 3D空間(現行 `/demo` 体験)をそのギャラリーのデータで開き、作品の追加・入替・空間設定変更 → **保存**
- **保存と公開**: 保存 = DBへ書き込み。公開トグルON = `/@username/slug` で公開、OFF = 非公開(404)
  - 公開中のギャラリーは保存すると即反映(スナップショット方式は廃止)(TBD-3)
- 削除(確認ダイアログ必須)。複製はTBD-4

### 10.3 作品ライブラリ

- 作品は**ギャラリーから独立したユーザー資産**(現行スキーマ通り)。複数のギャラリーで同じ作品を使い回せる
- ダッシュボードに「作品」タブ: 一覧 / アップロード / 削除(どこかのギャラリーで使用中なら警告)
- 上限: ストレージ容量 **300MB/ユーザー**(プラン変数)。動画は既存の40MB/本を踏襲

### 10.4 ダッシュボード(`/me`)

- **ギャラリー一覧**: サムネイル(代表作)/ タイトル / 公開状態バッジ / 更新日時
- 各ギャラリーの操作: 開く(3Dエディタ)/ 公開ページを見る / URLコピー / 名称変更 / 公開・非公開 / 削除
...
- `/@username`: プロフィール + **公開中のギャラリー一覧**(URL設計に既存・v0.4で実装)
- `/@username/[slug]`: 既存の3D鑑賞ページ(吊し方・キャプション対応済み)
- OGP: 既存方式(代表作画像)を踏襲。代表作の扱いはTBD-7

### 10.6 非機能・安全

- RLS: 既存方針を複数ギャラリーへ拡張(ownerのみ書き込み / 公開ギャラリーとその配置・作品のみ誰でも読める)
...
LP(/) ─→ 体験(/demo, ゲスト・localStorage)
      └→ 登録(/signup)→ ダッシュボード(/me)─→ 3Dエディタ(ギャラリーごと)─ 保存
                                         └→ 公開ページ(/@username/[slug])
```
...
| 2 | ギャラリーの上限 | **リリース時は1人1個**。設計は複数対応、プラン変数で拡張 |
...
| 9 | ギャラリーあたりの配置数 | **プラン変数**とする。初期値は決めうちで**10点**(10.10) |

### 10.9 既存実装からの移行メモ

- 現行: 空間設定はブラウザ(localStorage)にグローバル1つ。公開は `slug='main'` 固定で「現在の空間のスナップショット」を書き込む方式
- v0.4: **設定・配置はギャラリーごとにDBが正**となる。`/demo` はゲスト専用体験として現行方式を維持
- 既存ユーザーの `main` ギャラリーは「最初のギャラリー」としてそのまま引き継ぐ(データ変換不要、コード側の参照をgallery_id中心に変更)
...
| 変数 | 初期値(リリース時) | 備考 |
|---|---|---|
| `galleries` — ギャラリーの数 | 1 | DBは複数対応済み。超過時はダッシュボードで案内 |
| `worksPerGallery` — ギャラリーあたりの配置数(新規ギャラリーのデフォルト) | 5 | §11.5のフリー枠に合わせて改定(旧10)。実効スロット数 = min(レイアウトのスロット数, そのギャラリーの`work_cap`) |
...
- **Exploreフィード**(`/explore`): 公開中の全ユーザーのギャラリーを更新日時順に一覧表示し、他の人のギャラリーへ回遊できるように。
...
- 訪問者モードでは、左上の「← XIBIT360」ブランドリンクをやめ、小さな「XIBIT360」ホームリンク→ギャラリー名(タイトルが未設定のプレースホルダーなら代わりに作家名)→サブ行に作家名(タイトルと重複しない時だけ)・@ハンドル・SNSアイコン・Explore・Reportの導線を配置
...
- 変わっていないもの: 「Start free」はまだ本物の登録フォームへのリンクに過ぎず(サインアップ自体は既存の`/signup`をそのまま使用)、購入リンクは外部サイトへの単純なリンクであり決済機能はXibit360側には一切ない
...
ユーザー提案「3D空間なので、他の人の気配を作りたい。過去の訪問者数に応じて非同期で見えると良い」。リアルタイム同時接続(=以前フェーズ2に後回しにしたマルチプレイ)ではなく、**過去の累計訪問数に応じた半透明シルエットが会場を徘徊する**非同期プレゼンスとして実装。人気のギャラリーほど賑わって見え、そのまま社会的証明になる:
...
- **訪問数の公開読み取り**(migration `0024`): `visits`のSELECTはオーナー限定(0008)なので、**集計だけを返すSECURITY DEFINER RPC `public_visit_count`**を追加(公開ギャラリーのみ・個票/時刻は返さない・非公開は0)。`fetchPublicExhibition`が読み`PublicExhibition.visitCount`へ
...
- **作品ごとの表示価格(自由入力テキスト)**。Xibit360は決済を代行しないので、作家が入力したそのまま(`¥50,000`/`$500`/`Ask`等・通貨や書式は自由)を作品パネルに表示するだけ。
