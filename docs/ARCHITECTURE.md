...
3. **公開** — `@username/slug` 公開URL、公開トグル、OGP画像生成
   → **実装済み(v0.3)**。設定パネルの「公開」からユーザー名設定 → 公開/更新/非公開。
   `/[handle]/[slug]` がSSRでOGP(タイトル + 代表作画像)を返し、来場者は読み取り専用の
   visitor モードで鑑賞。OG画像はまず代表作をそのまま使用(専用カード生成は今後)
4. **エディタ強化** — 配置入れ替え、ギャラリー複数管理、プロフィール編集
5. **(以降)** Meta認証・IG連携、アクセス解析、芳名帳 … 要件フェーズ2へ

補足(v0.3で追加実装):
- **動画作品(リール等)**: `artworks.kind='video'`。アップロードは40MB上限・要ログイン、
  Storageに `video` + ポスター `thumb.jpg` を保存。展示は VideoTexture でループ再生し、
  `THREE.PositionalAudio` で「近づくほど音が大きくなる」空間オーディオ。
  同時再生はカメラに近い2点(タッチ端末1点)のみ(lib/videohub.ts)
- 既知の落とし穴として、展示入替でシャドウ付きライトを作り直すと three がプログラムを
  再利用してシャドウサンプラ不整合になるため、構成変更時に全マテリアルを
  needsUpdate している(GalleryScene.tsx 参照)
- **空気感の演出(質感v2)**: 本物のボリューメトリックライト(レイマーチング)は
  ブラウザには重いため、加算合成の円錐メッシュで光のシャフトを擬似再現している
  (LightCone.tsx。1本あたり三角形数十枚で実質無コスト)。テーマごとに霧の濃度
  (`fogDensity`)と光錐の濃さ(`coneOpacity`)を持たせ、ノワールは濃く/
  ホワイトキューブは薄く空気遠近感を出す。額縁は仕上げ別(木目/ブラシ研磨/塗装)の
  手続き生成 bump+roughness マップで均一な艶(=プラスチック感)を回避
  (textures.ts の getFrameFinish)
