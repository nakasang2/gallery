...
- 決定1（鑑賞ビュー）: **詳細パネルに作品の図版を表示し、クリックで全画面ライトボックス**。画質は当面**現状の1600pxのまま**（3D負荷対策で高解像度版の別保存＝案Bは見送り、まず「クリックで鑑賞できる土台」を作る）。図版ソースは アップロード作品=display画像 / 動画作品=poster / デモ生成作品=`renderArtworkCanvas(art,2048)`を都度描画（生成物は解像度非依存なので3D壁の1024より高精細）。
- 決定2（ステッパー）: **作品フォーカス中はオンキャンバスの`HudStepper`を隠し、Prev/Nextを詳細パネル内に移設**（`‹ No.NN / TT ›`のナビ行）。従来の`lifted`（スマホbottom sheet上へ持ち上げ）ロジック／CSSは不要になり撤去。
- 却下: 案B(高解像度版を別保存してクリック時ズーム)=ストレージ増＋アップロード改修のため今回見送り（将来必要になれば再検討）。案C(3Dテクスチャ上限の一律引き上げ)=VRAM/帯域増でモバイル描画悪化、かつ「被り」「鑑賞ビュー無し」は解決しないため却下。
- 対象: `components/gallery/ArtworkPanel.tsx`（図版・ライトボックス・パネル内Prev/Next）、`components/gallery/Hud.tsx`（`HudStepper`をfocus中tuck）、`app/gallery.css`（`.panel-nav`/`.panel-figure`/`.lightbox`追加、旧`.hud-stepper.lifted`撤去）。/demoで実機確認、tscクリーン。
