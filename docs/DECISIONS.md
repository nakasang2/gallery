...
- **プレビュー(devサーバー)を止めてから build する**（3回ルール ×3 → 2026-07-28 昇格）。同じ `.next` を両方が触ると `Cannot find module './331.js'` で全ページが白画面になる。`prebuild` フックが `.claude/launch.json` のポートを検査して止める。
- **push前に `npm run check:i18n` と `npm run check:css` を通す**（AGENTS.md 5 / 5.1）。
