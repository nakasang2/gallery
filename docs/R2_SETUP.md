...
### 削除した作品が最大4時間キャッシュから取得できる（未対応）

DBとR2からは即座に消えるが、CloudflareのCDNキャッシュには残る。URLを知っている人は
その間アクセスできる。テイクダウン対応を厳密にするなら、削除時にCloudflareの
キャッシュパージAPIを呼ぶ必要がある（ゾーン権限のAPIトークンが別途必要）。
