...
- **最終更新**: 2026-07-24（3Dプレビューに導入ドリーイン追加＝引いた視点から作品へ寄る「つかみ」）
...
## 完了ログ（直近5件）
- 2026-07-24: **作品の3Dプレビューモード＋導入ドリーイン**（DECISIONS 2026-07-24参照）。詳細パネルの「View in 3D」ボタンで、作品を額縁ごと無地スタジオに浮かべ**ドラッグで自由回転＋ホイール/ピンチで拡大**できる全画面プレビューを実装（新規`ArtworkPreview3D.tsx`＝遅延ロードの専用Canvas＋drei OrbitControls、`Exhibit`の`makeFrameGeo`/`exhibitExtents`再利用でギャラリーと同じ額装、動画作品はposter）。開いた瞬間は**引いた斜め上→正面へ1.15秒で寄るドリーイン**（`IntroDolly`、完了後にOrbitControlsをマウント、reduced-motion配慮）。UI文言は英語統一。/demoで起動・ドリーイン・回転・ズーム・クローズを実機確認、tscクリーン。
