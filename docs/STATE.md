...
- **最終更新**: 2026-07-24（2Dパネル図版は撤去し3Dプレビューモードへ方針転換。Prev/Nextパネル内移設は維持）

## 進行中
- **作品の3Dプレビューモード**: 作品クリック時の鑑賞を、ギャラリー空間とは別の「拡大・回転できる専用3Dビュー」で提供する構想（ユーザー要望2026-07-24）。実装アプローチをユーザーへ提示し選択待ち。既存`components/Preview3D.tsx`（ダッシュボードの部屋プレビュー）が参考・流用候補。
