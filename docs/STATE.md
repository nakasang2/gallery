...
- **最終更新**: 2026-07-17（作品の実寸・画材＋3D比率反映 §11.21。プリセット選択→即時プレビュー→ダッシュボードのスケール基準人型を実glTFモデルに置換 §11.21.4→壁からの距離・カメラフィットを是正 §11.21.5＋Size入力行のUI改行修正 §11.21.6）
