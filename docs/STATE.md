...
- **最終更新**: 2026-07-21（空間BGM: WebAudio生成の1トラックloopをlib/audio.tsに追加。P3-12完了）
...
- docs/STRATEGY.md **§7** の残タスク:
  ...
  - P3-12 音声ガイドは**作品ごとガイドを実装済**(migration 0021)。**空間BGMも実装済**(2026-07-21)
...
## 完了ログ（直近5件）
- 2026-07-21: **空間BGM（STRATEGY §7 P3-12 残タスク解消）**。既存の`lib/audio.ts`（ルームトーン/足音/雑踏ノイズすべてWebAudio生成・音声ファイル無し）の流儀を踏襲し、**単一トラックの生成的アンビエントBGMループ**を追加。ペンタトニック風スケールから疎にノートを選び、各ノートをHann窓(フェードイン/アウト)で包んで重ね書きすることで、ノート同士が重なってもクリックノイズが出ず、かつバッファの始端/終端に必ず無音区間が残る＝**ループ境界が完全にシームレス**になる設計(Eno的な非機械的反復感)。既存の`master`ゲイン(mute/退出suspend)にそのままぶら下げたので**新規トグル不要**、`galleryAudio.enabled`一本の制御をそのまま継承。検証: バッファ生成ロジックをNodeで30回ランダム実行しNaN無し・両端無音・ピーク振幅<1.0(最終音量`BGM_LEVEL=0.055`適用で明らかに歪まない)を確認、加えて実Chromium(`/opt/pw-browsers/chromium`)の実`AudioContext`上でも同一コードが例外無く構築・接続できることを確認。tsc・build クリーン
