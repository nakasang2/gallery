...
- **最終更新**: 2026-07-27（UX/UI/プロダクト/法務のレビューを実施。指摘のうちモバイル4件をコミット済・未push）／別セッションで3Dモデル等のR2配信化をship（本番反映確認済み）

## 進行中
- **【要判断・3回ルール到達】「CSSが書いてあるのに効いていない」型が1セッションで×3**（LPナビの導線コメントと実結果の乖離 / `viewport-fit`未設定で safe-area 14箇所が無効 / `.hud-actions`・`.hud-record` という存在しないクラス名）。LESSONS.md に記録済み。AGENTS.md の3回ルールに従い、**絶対ルール化かスキル化をユーザーに提案する必要がある**（勝手に恒久ルール化しない）。/kaizen で棚卸しする。
- **【コミット済・未push】レビュー指摘のモバイル6件**（`e28fd19` `a04a3d7` `9703ffd` `fa0c8b0`）。追加2件: `viewport-fit=cover`でsafe-areaを有効化（+ 非ギャラリーページのベースラインinsetと、fixed chromeのopt-in）／死にセレクタ`.hud-actions`を`.hud-cluster`へ修正しHUDのモバイル配置を復活（`.hud-record`は役目を終えたため削除）。360/390pxで矩形交差ゼロを実測。**safe-areaの実挙動はノッチ実機でしか確認できない（プレビューはinsetが常に0）→ 本番QAで要確認**。
- **【コミット済・未push】レビュー指摘のモバイル4件**（`e28fd19` `a04a3d7` `9703ffd`）。①電話幅でLPの「Start free」が画面外に切れていた ②縦画面のFOV固定で横視野が1/3に潰れていた ③ローディングがタイマーで「終わったふり」をしていた ④作品画像の二重JPEG圧縮。tsc/buildクリーン、320〜1280pxで実機幅の実測・スクショ確認済み。**残: push（未実施）と、③のスロー回線での実挙動（ローカルはキャッシュ済みで再現不可＝本番の実機QAが必要）／④のサインイン後アップロード実測**。
- **【要判断】レビューで挙がった未着手の指摘**。優先度順に整理済み（このセッションの報告参照）: (Phase 0) 購入台帳に`currency`列が無くStripeが現地通貨で提示すると売上集計が壊れる／事業者表記・返金方針・年齢の定めが規約に無い（特商法・EU撤回権・未成年者取消権）／サインアップに規約同意チェックが無い／LPが「only Video Pass renews yearly」と自らサブスクを謳っている（Video Passは非販売）。(Phase 2) `/admin`に通報一覧も非公開化ボタンも無い（reportsはSELECTポリシー自体が無く件数表示のみ）／芳名帳が匿名・無審査・ON/OFF不可／公開＝Explore自動掲載で限定公開が選べない／本番の`/explore`が0件。(Phase 3) i18n基盤ゼロ・sitemap/robots無し／CJKタイトルで3Dプレート・タイトル壁が溢れる。
