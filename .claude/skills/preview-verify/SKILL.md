...
このリポジトリでは **「検証ツールに嘘をつかれて、実装のバグだと誤診する」が ×12 起きた**（/kaizen 昇格 2026-07-29 の時点で ×6。**昇格後も ×7 → ×12 と増え続けている**）。毎回10分〜数時間を溶かし、**2回は誤診に基づいて別のバグを作った**。ユーザーは非エンジニアで本番の唯一のQAなので、未検証・誤検証のまま「修正しました」と報告すると本番での手戻りに直結する。
...
| **スクロールした位置のスクリーンショットが真っ黒** | `window.scrollTo` で移した位置はキャプチャに反映されず、塗られていない面が返る（DOMは正しい ── 2026-08-20 に実測）。`computer{action:'scroll'}` はペインが hidden だとタイムアウトする | **スクロールで見るのをやめ、`resize_window` でビューポートを縦に伸ばして1枚に収める**（縦2400pxで管理画面3節を1枚に）。3Dのように伸ばせない面は**撮れないと認めて実機QAへ回す** |
