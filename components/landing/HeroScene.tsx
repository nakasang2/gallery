...
// 最初 18→170 で試したところ、実機（スマホ実測）でフォーカスの窓を外れた
// 位置が「暗すぎて見えない」との指摘を受けた。指でのスクロールは狙った1px
// で止まらないので、素通り域の時間の方が長い ── そこが暗すぎると本文が
// 常に読めなくなる。**素通り時も旧来の常時点灯(130)より明るい水準を保ち**、
// フォーカス時はそこからさらに一段明るくする、という控えめな効かせ方にした。
const PANEL_LIGHT_BASE = 100
const PANEL_LIGHT_FOCUS = 190
