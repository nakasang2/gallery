...
// フォールバック)。中央/左は HERO_ART と同じ位置で、右は主役より手前に置くと画面の
// 右へ寄る。HERO_ART はカメラの止まり位置(anchor)も兼ねるので描画用は別で持つ。
//
// **右の z と scale は必ず対で動かす**(ユーザー指摘 2026-08-19「一番手前のアートだけ
// サイズ感が大きい・位置も中央の作品に寄っている」)。壁に貼ってあるので**右へ離すほど
// 必ずカメラに近づいて大きくなる**ため、離した量を scale で打ち消さないと太る。
// 変更前 (z 9.2 / scale 1.3) は入口ビューで**中央の1.13〜1.20倍**の幅で出ており、
// 中央とのすき間も左側のちょうど半分だった(PC 1280x800 で 183px 対 345px)。
// 現在の値は幅を中央と同じにし、すき間を左右ほぼ対称にしたもの(PC 339px 対 345px)。
// ※ scale は世界の寸法(幅 = 1.5 × scale)で、画面に出る大きさはカメラからの距離で決まる。
//   数字を変えたら scripts/ ではなく実機/ペインの入口ビューで見た目を確かめる。
const HERO_SLOTS: { idx: number; z: number; scale: number }[] = [
  { idx: 0, z: 6, scale: 1.35 }, // center
  { idx: 5, z: 1, scale: 1.35 }, // left
  { idx: 6, z: 10, scale: 1.1 }, // right — z と scale は対で(上のコメント)
]
