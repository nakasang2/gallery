...
--
-- **【2026-08-15】既に 0062 を適用済みのデータベースには、このファイルを当てないこと。**
-- 代わりに `0065_notify_unsubmit_retry.sql` を使う。下の制約は 0062 が広げた一覧の
-- **部分集合**（`room_ready` が無い）なので、0062 のあとに当てると許可済みの種別を
-- 取り上げる後退になり、実際に本番で
--   ERROR: 23514: check constraint "notifications_kind_check" ... is violated by some row
-- で弾かれた。**番号順に流す経路（schema.sql・新しい環境）ではこのファイルのままで正しい。**
--
