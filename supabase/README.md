...
2. `supabase/migrations/` のSQLを**番号順に**全文貼り付けて **Run**
   ...
   - `0061_expo_room_scope.sql` — **合同展示を1展示1部屋にし、作品を専用プールへ分離する**（ユーザー決定 2026-08-13）。部屋を合同展示に切り替えると口座全体の作品ライブラリ（他の通常展示の部屋とも共有）がそのまま並び、「今まで使っている設定が反映されている」ように見えて混乱するという指摘があった。①`enforce_room_allowance`（新規部屋のINSERT経路）と`switch_room_expo`（既存部屋の切替経路）の両方に「その展示に既に部屋が無いか」を足し、1展示につき1部屋までに制限する。②`artworks.gallery_id`（合同展示の部屋のidだけを名乗れる。通常展示の部屋のidや他人の部屋のidを名乗ると拒否）を新設し、`work_slot_pool`/`placement_pool_used`（0059）を**合同展示の部屋を除いて**数えるよう変更。合同展示の部屋自身の作品・配置は、口座の共有プールへは足さない・引かない、その部屋自身の`work_cap`（15）だけで独立に完結する。2026-08-12の決定（「作品スロットの合計は個人の部屋⇄合同展示の部屋の間でも成り立つ」）を一部撤回するもの。SQLテスト19項目。
   - `0062_expo_participant_rooms.sql` — **合同展示を「参加作家ごとに部屋」の形に直す**（ユーザー決定 2026-08-13）。招待→承諾のあと作家が触るUIが「受信箱の中の作品トレイ」という特殊な形で、通常の部屋編集（作品／部屋／配置／公開のタブ）と操作感が違いすぎるという指摘があった。**0061 の「1展示1部屋」を「1展示・1作家1部屋」に訂正**（主催者も招待作家も同じ部屋を1つずつ持つ）。①`enforce_room_allowance`/`switch_room_expo` の所有チェックを緩め、承諾済みの招待がある作家も自分の部屋を作れるようにし、上限判定を「展示全体に部屋が無いか」から「自分がその展示に部屋を持っていないか」に変更。②新規トリガ `create_room_on_expo_accept`（`expo_invites` の insert/update of status）が、招待が `accepted` になった瞬間にその作家自身の部屋（`work_cap=15`）を自動で作る（冪等）。③`drop_placements_on_expo_revoke` に「作家自身の部屋を削除する」を追加（辞退・取り消しで部屋も消える。アップロード済みの作品は `artworks.gallery_id` の `on delete set null`（cascadeから変更）で失われず共有プールへ解放される）。④`galleries.expo_ready_at` と RPC `set_expo_room_ready` を新設。作家が自分の合同展示の部屋で「準備できた」トグルをオンにすると主催者へ`room_ready`通知が届く（自己通知はしない）。⑤新規ポリシー `galleries_select_expo_owner` で、展示の主催者が展示内の全部屋（他の作家の部屋を含む）を読めるようにする（書き込みはできない）。旧来の「作家が出す作品を選ぶ→主催者が壁に掛ける」仕組み（`expo_submissions`）は表・関数とも残す（既存データを壊さない）が、アプリからは呼ばなくなる想定。SQLテスト21項目。
   - `0063_expo_leave_guard.sql` — **参加作家が「通常展示に戻す」で合同展示から消えるのを塞ぐ**（ユーザー決定 2026-08-14）。`RoomExpoBadge` の「通常展示に戻す」は名前に反して**変換**で、参加作家が押すと自分の合同展示の部屋が通常展示の部屋に変わり展示から居なくなる。招待は `accepted` のまま残るので 0062 の `create_room_on_expo_accept` は二度と発火せず、**復帰する導線が1つも無い**（バッジからの参加は 2026-08-12 に撤去済み・受信箱の「部屋を開く」も出ない）。本番で実際に1人が消えた（残った部屋の `slug` が自動生成名のまま `expo_id` だけ null、`created_at` が承認時刻とマイクロ秒まで一致）。`switch_room_expo` に「`p_expo is null`（展示から外す向き）のとき、その展示の主催者でなければ拒否」を1つ足すだけで、**主催者には残す**（主催者は `ExpoManager` の「部屋を追加」で取り返しがつくが、参加作家には無い）。降りる道は受信箱の「降りる」＝辞退（0062 のトリガが部屋も片付ける）に一本化する。SQLテスト13項目。**番人を外すと項目02・03が赤くなり、03は「部屋が展示から消えた」＝本番の事故そのものを再現することも実測。**
   - `0064_expo_ready_room_visible.sql` — **「準備できた」参加作家の部屋だけ主催者に中身を見せる**（ユーザー選択C 2026-08-14）。0062 の `galleries_select_expo_owner` が主催者に開けたのは**部屋の行だけ**で、作品と配置は開けていなかった ── 実測すると「会期前は主催者も一切読めない／会期が始まると全員読める」で、**主催者は中身を一度も見ないまま $15〜$40 の場所代を払う**構図だった。述語 `expo_room_ready_for_organizer()` を1つ置き、`artworks` と `placements` の select ポリシーが**同じ述語**を読む（同じ条件を2か所に書かない）。作家が `set_expo_room_ready` をオフに戻せばまた見えなくなる＝**見せるかどうかの決定権は最後まで作家が持つ**。**書き込みのポリシーは1つも増やしていない**（テスト07〜10で再確認）。SQLテスト19項目。**述語の execute は `anon` にも配る** — RLSのポリシーは全ロールで評価されるので、絞ると**未ログインの来場者が `artworks` を1行読むだけで `permission denied for function` で落ちる**（0041 と同じ形の事故を実際に書いてしまい、テスト13が検出した）。未ログインでは `auth.uid()` が null なので述語は常に false を返す＝配っても見えるものは増えない。**ポリシーを外すと項目05・06が赤くなることも実測。**
   - `0065_notify_unsubmit_retry.sql` — **0057 の関数とトリガだけを、あとから当てられる形で置き直す**（2026-08-15）。`0057_notify_unsubmit.sql` を本番へ貼ったら `ERROR: 23514: check constraint "notifications_kind_check" ... is violated by some row` で弾かれた。**中身は正しく、順序だけが問題**で、0057 の一覧（`unsubmit` まで）は 0062 の一覧（`unsubmit` ＋ `room_ready`）の**部分集合**なので、0062 を先に当てた本番に 0057 を当てると**許可済みの `room_ready` を取り上げる後退**になり、実際に飛んでいた `room_ready` の通知行に弾かれた。0065 は**制約を一切触らず**関数とトリガだけを置く。番号順に流す経路ではこの3本（0057→0062→0065）がこの順に走り、0065 は同じ関数を `create or replace` するだけなので結果は変わらない。**列挙を丸ごと書き換える migration は、順序を守れないと二度と当てられなくなる**ことの実例。
   - `0066_drop_design_colours.sql` — **Design Tools（壁・床・光の色と光の強さ）の保存済みの値を消す**（ユーザー決定 2026-08-17）。部屋タブを「選ぶだけの場所」にするため画面から撤去した。**アプリ側は同じ日に `resolveTheme` から色の上書きを外してあるので、当てなくても見た目はテーマどおりに戻る** ── ここで消すのは残った値そのもので、放置すると「画面には無いのにDBには入っている」状態が続き、次に `design_overrides` を触る人が必ず調べ直すことになる。`lightMode`（部屋ごとの照明）も同時に消す（作品ごとの上書きは `artworks` 側にあり触らない）。`logoUrl` は色ではなく作家の持ち物なので残す。DDLは無く `update` 1本だけ。再実行安全。
   - `0067_gallery_shadow_bake.sql` — **焼いた壁の影（アトラス1枚）の保存先 `galleries.shadow_bake` を足す**（DECISIONS 2026-08-18）。列を1つ足すだけで、当てなくても今までどおり来場者のブラウザが毎回焼く（保存されないだけ）。
   - `0068_guard_theme_layout.sql` — **有料テーマ/レイアウトを購入なしで設定できる穴を塞ぐ**（別視点レビュー 2026-08-20・リリース前監査で発見）。`galleries_owner_all`（0001）は`owner_id`しか見ておらず、`theme`/`layout`列の値を検査していなかったため、ログイン済みユーザーが直接`update`を送るだけで有料テーマ/レイアウトを無料設定できた。`work_cap`（0036）等と同じ形の列専用ガードトリガ（`galleries_guard_theme_layout`）を追加。購入済み、または**新規作成(INSERT)に限り**自分の別の部屋に既にその値がある（2室目以降はメイン部屋の見た目をコピーする設計のため）場合のみ許可する（UPDATEにも効かせると未購入の値を全部屋へ複製できてしまうため限定した）。
   - `0069_purchases_retain_on_delete.sql` — **アカウント削除で購入履歴が消えるのを止める**（別視点レビュー 2026-08-20・リリース前監査で発見）。`purchases.user_id`が`on delete cascade`だったため退会すると決済記録が全消去され、`/privacy`の保持ポリシーと矛盾していた。`artworks.gallery_id`（0062）と同じ形で`on delete set null`に変更し、行（金額・SKU等）は残したまま個人識別子だけを外す（匿名化）。保持期間そのものは要弁護士確認。
   - `0070_capacity_purchase_delta.sql` — **容量購入がクランプされて一部しか反映されなくても'applied'を返していた穴を塞ぐ**（リリース前監査で発見・2026-08-20）。`record_capacity_purchase`は「行が更新されたか」しか見ておらず、部屋が15枠の上限近くでほぼ同時に2件の容量購入が届くと、2件目は実増分0でも成功扱いになっていた。更新前のwork_capを`for update`で行ロックしながら読み、実際の増分が要求量に届かない場合は新しい戻り値`'partial'`を返す（webhook側はこれをログして手動突き合わせに回す）。
   - `0071_rate_limits.sql` — **汎用のレート制限カウンタ**（リリース前監査 #11・#31・ユーザー決定 2026-08-21）。外部サービス（Redis等）を新たに契約せず、既存の`reserve_storage`（0030）と同じ「Postgresの関数+テーブルで数える」作法に揃える。`rate_limit_counters`（bucket, key, window_start ごとに件数を数える固定窓方式）はRLS有効・ポリシー無しで直接の読み書きを塞ぎ、唯一の入口`check_rate_limit()`（security definer）は`authenticated`と`service_role`だけが呼べる。
